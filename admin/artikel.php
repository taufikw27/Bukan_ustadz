...
if (isset($_POST['add'])) {
	...
	{
	global $connect;
		$sql = "INSERT INTO artikel (Judul,Isi,Kategori,Gambar,Teks,Tanggal,Viewnum,Updateby,Post_type,Terbit) VALUES ('".$_POST['judul']."','".$_POST['isi']."','".$_POST['kategori']."','$gambar','".$_POST['teks']."','".date("Y-m-d H:i:s")."','0','".$_SESSION['loginadmin']."','berita','".$_POST['terbit']."')";
	...
	{
		global $connect;
		$sql = mysqli_query($connect,"UPDATE artikel SET Judul='".$_POST['judul']."',Isi='".$_POST['isi']."',Kategori='".$_POST['kategori']."',Gambar='$gambar',Teks='".$_POST['teks']."',Terbit='".$_POST['terbit']."' WHERE ID='".$_POST['id']."' ");
	}
...
if(isset($_GET['act']) && $_GET['act']=='edit'){
	...
	while($b = mysqli_fetch_array($sql)){
		...
		{
			unlink('../'.$gambar);
			$sqlupdate = mysqli_query($connect,"UPDATE artikel SET Gambar='' WHERE ID='$id' ");

			echo'<meta http-equiv="REFRESH" content="0;url=./?mod=artikel&act=edit&id='.$id.'" />';
		}
...
if(isset($_GET['act']) && $_GET['act']=='hapus'){
	...
	$hapus = mysqli_query($connect,"DELETE FROM artikel WHERE ID='$id' ");

	}

?>
<div class="w100">
	<form action="./?mod=artikel" method="POST" enctype="multipart/form-data">
		...
		<fieldset>
			...
			<input type="text" name="judul" placeholder="Judul Artikel" value="<?=(isset($judul) ? $judul : '')?>" size="40">
			</div>
			...
			if(!empty($gambar) && !empty($id) ){
				...
				<div class="imghapus"><a href="./?mod=artikel&act=edit&hapusgambar=yes&id='.$id.'">x</a></div>
				</div>
...
<div class="w100">
	<fieldset>
		...
		global $connect;
		$i =1;
		$hasil = mysqli_query($connect,"SELECT * FROM artikel ORDER BY ID DESC");
		while ($b = mysqli_fetch_array($hasil)) {
			...
			<div class="w20 fl"><?=$Tanggal;?></div>
			<div class="w10 fl">
				<a href="./?mod=artikel&act=edit&id=<?=$ID;?>" class="btn-primary pd5">edit</a>
				<a href="./?mod=artikel&act=hapus&id=<?=$ID;?>" class="btn-red pd5">hapus</a>
